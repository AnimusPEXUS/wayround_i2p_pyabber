...
class SubjectEditor:
    ...
    def __init__(self, controller, operation_mode='chat'):
        ...
        editor = wayround_i2p.pyabber.message_edit_widget.MessageEdit(
            controller,
            mode='subject'
            )
...
#        self._tooltip.destroy()
    ...
    def _on_delete_button_clicked(self, button):
        ...
        if self._operation_mode != 'normal':
            ...
            s = wayround_i2p.xmpp.core.Stanza(tag='message')
            s.set_to_jid(self._contact_bare_jid)
            s.set_subject([wayround_i2p.xmpp.core.MessageSubject(None)])
    ...
    def _on_edit_button_clicked(self, button):
        ...
        j = wayround_i2p.xmpp.core.JID.new_from_string(self._contact_bare_jid)
        if self._operation_mode != 'groupchat':
