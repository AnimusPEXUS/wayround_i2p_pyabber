...
class Item:
    ...
    check_nick = wayround_i2p.xmpp.muc.Item.check_nick
    check_affiliation = wayround_i2p.xmpp.muc.Item.check_affiliation
    check_role = wayround_i2p.xmpp.muc.Item.check_role
    check_jid = wayround_i2p.xmpp.muc.Item.check_jid
    ...
    def check_show(self, value):
        if value is not None and not value in ['available', 'unavailable']:
            wayround_i2p.xmpp.core.PresenceShow.check_text(self, value)
...
class Storage:
    ...
    def __init__(self, room_jid, presence_client):
        ...
        if not isinstance(presence_client, wayround_i2p.xmpp.client.Presence):
            raise TypeError(
                "`presence_client' must be wayround_i2p.xmpp.client.Presence"
                )

        if not isinstance(room_jid, wayround_i2p.xmpp.core.JID):
            raise TypeError(
                "`room_jid' must be wayround_i2p.xmpp.core.JID"
                )
    ...
    def _on_presence(
        ...
        with self._lock:
            ...
            if event == 'presence':
                ...
                fj = wayround_i2p.xmpp.core.JID.new_from_str(from_jid)
                ...
                if fj.bare() == self._room_jid.bare():
                    ...
                    muc_elem_list = wayround_i2p.xmpp.muc.get_muc_elements(
                        stanza.get_element()
                        )
                    ...
                    if len_muc_elem_list == 1:
                        ...
                        if e.tag == \
                            ...
                            muc_obj = \
                                wayround_i2p.xmpp.muc.X.new_from_element(e)
