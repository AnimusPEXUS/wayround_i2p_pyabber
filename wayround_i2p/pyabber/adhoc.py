...
import wayround_i2p.xmpp.adhoc
...
class AD_HOC_Window:
    ...
    def _on_ok_button_clicked(self, button):
        ...
        if not self._selected_command:
            ...
            com = wayround_i2p.xmpp.adhoc.Command(
...
class AD_HOC_Response_Window:
    ...
    def run(self, stanza_response, command_struct):
        ...
        if not isinstance(stanza_response, wayround_i2p.xmpp.core.Stanza):
            raise TypeError(
                "`stanza_response' must be wayround_i2p.xmpp.core.Stanza"
                )

        if not isinstance(command_struct, wayround_i2p.xmpp.adhoc.Command):
            raise TypeError(
                "`command_struct' must be wayround_i2p.xmpp.adhoc.Command"
                )
...
"if You see this message"
    ...
    def _add_note(self, note):
        ...
        if not isinstance(note, wayround_i2p.xmpp.adhoc.CommandNote):
            raise TypeError(
                "`note' must be wayround_i2p.xmpp.adhoc.CommandNote"
                )
    ...
    def _add_x_form(self, data, stanza):
        ...
        if not isinstance(data, wayround_i2p.xmpp.xdata.XData):
            raise TypeError("`data' must be wayround_i2p.xmpp.xdata.XData")

        res = wayround_i2p.pyabber.xdata.XDataFormWidget(
            self._controller,
            data,
            stanza
            )
        ...
        if not self._form_controller:
            ...
            if self._command_struct.get_status() != 'executing':
                ...
                if x_data == None:
                    ...
                    command = wayround_i2p.xmpp.adhoc.Command()
                    ...
                    stanza = wayround_i2p.xmpp.core.Stanza(tag='iq')
                    stanza.set_typ('set')
...
def adhoc_window_for_jid_and_node(to_jid, controller):
    ...
    commands = wayround_i2p.xmpp.adhoc.get_commands_list(
...
def process_command_stanza_result(res, controller):
    ...
    if not isinstance(res, wayround_i2p.xmpp.core.Stanza):
        ...
        if res.is_error():
            ...
            commands = wayround_i2p.xmpp.adhoc.extract_element_commands(
                res.get_element()
                )
...
process_command_stanza_result_idled = \
    wayround_i2p.utils.gtk.to_idle(process_command_stanza_result)
