...
import wayround_i2p.pyabber.connection_window
import wayround_i2p.pyabber.icondb
import wayround_i2p.pyabber.profile_window
import wayround_i2p.pyabber.status_icon
...
class Main:
    ...
    def _profile_selection_dialog_constructor(self):
        return wayround_i2p.pyabber.profile_window.ProfileMgrWindow(self)

    def run(self):
        if not self._working:
            self._working = True
            self.status_icon = wayround_i2p.pyabber.status_icon.MainStatusIcon(
                self
                )
...
class ProfileSession:
    ...
    def _connection_mgr_dialog_constructor(self):
        return wayround_i2p.pyabber.connection_window.ConnectionMgrWindow(
            self._main, self
            )
...
#    GObject.threads_init()
    ...
    wayround_i2p.pyabber.icondb.set_dir(
        wayround_i2p.utils.path.join(
            os.path.dirname(wayround_i2p.utils.path.abspath(__file__)),
            'icons'
            )
        )
