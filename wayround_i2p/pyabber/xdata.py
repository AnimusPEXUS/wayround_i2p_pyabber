...
import wayround_i2p.pyabber.xdata_media_element
...
class XDataFormWidget:
    ...
    def __init__(self, controller, x_data, origin_stanza, editable=True):
        ...
        if not isinstance(x_data, wayround_i2p.xmpp.xdata.XData):
            raise TypeError("`x_data' must be wayround_i2p.xmpp.xdata.XData")
        ...
        if not isinstance(
            ...
            raise TypeError(
    "`controller' must be wayround_i2p.pyabber.ccc.ClientConnectionController"
                )

        if (origin_stanza != None
            and not isinstance(origin_stanza, wayround_i2p.xmpp.core.Stanza)):
            raise ValueError(
                "`origin_stanza' must be None or wayround_i2p.xmpp.core.Stanza"
                )
    ...
    def field_widget(self, field_data):
        ...
        if not isinstance(field_data, wayround_i2p.xmpp.xdata.XDataField):
            raise TypeError(
                "`field_data' must be wayround_i2p.xmpp.xdata.XDataField"
                )
        ...
        if media != None:
            media_widget = \
                wayround_i2p.pyabber.xdata_media_element.MediaElementWidget(
                    self._controller,
                    media,
                    self._origin_stanza
                    )
...
class FieldBoolean:
    ...
    def get_values(self):
        ...
        ret = wayround_i2p.xmpp.xdata.XDataValue(ret)
        return [ret]
...
class FieldList:
    ...
    def get_values(self):
        ...
        for i in rows:
            ne = model[i][0]
            ret.append(wayround_i2p.xmpp.xdata.XDataValue(ne))
...
class FieldTextArea:
    ...
    def get_values(self):
        ...
        if self._is_jid_mode:
            ...
            lines = wayround_i2p.utils.list.list_lower(lines)
            lines = wayround_i2p.utils.list.\
        ...
        ret = []
        for i in lines:
            ret.append(wayround_i2p.xmpp.xdata.XDataValue(i))
    ...
    def has_errors(self):
        ...
        if not self._is_jid_mode:
            ...
            for i in val:
                ...
                err = False
                try:
                    res = wayround_i2p.xmpp.core.JID.new_from_str(
                        i.get_value()
                        )
                except:
                ...
                if not isinstance(res, wayround_i2p.xmpp.core.JID) or err:
                    ret = True
...
class FieldText:
    ...
    def get_values(self):
        ...
        return [wayround_i2p.xmpp.xdata.XDataValue(ret)]
    ...
    def has_errors(self):
        ...
        if not self._is_jid_mode:
            ...
            err = False
            try:
                res = wayround_i2p.xmpp.core.JID.new_from_str(val.get_value())
            except:
                logging.exception("Can't convert string to JID")
                err = True
...
class ReportWidget:
    ...
    def __init__(self, x_data):
        ...
        if not isinstance(x_data, wayround_i2p.xmpp.xdata.XData):
            raise TypeError(
                "`field_data' must be wayround_i2p.xmpp.xdata.XData"
                )
