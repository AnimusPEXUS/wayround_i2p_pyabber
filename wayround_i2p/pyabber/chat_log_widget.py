...
import wayround_i2p.pyabber.chat_pager
import wayround_i2p.pyabber.l10n
import wayround_i2p.pyabber.message_filter
...
class ChatLogTableRow:
    ...
    def get_mode_lang_switch_status(self):
        ...
        if active != -1:
            ...
            res = wayround_i2p.pyabber.l10n.LANG_MODE_RE.match(active_text)
...
class ChatLogWidget:
    ...
    def __init__(
        ...
        if not isinstance(chat, wayround_i2p.pyabber.chat_pager.Chat):
            raise TypeError(
                "`page' must be the instance of "
                "wayround_i2p.pyabber.chat_pager.Chat"
                )

        self.add_record_idle = wayround_i2p.utils.gtk.to_idle(self.add_record)
        ...
        self.scroll_down_idle = \
            wayround_i2p.utils.gtk.to_idle(self.scroll_down)

        self._looped_timer = wayround_i2p.utils.timer.LoopedTimer(
            0.25,
            self.scroll_down_idle,
            tuple(),
            dict()
        ...
        with self._incomming_messages_lock:
            ...
            if event == 'new_message':
                if type_ in ['message_chat', 'message_groupchat']:
                    ...
                    if wayround_i2p.pyabber.message_filter.\
    ...
    def load_history(self):
        ...
        jid_resource, types_to_load = \
            wayround_i2p.pyabber.message_filter.\
