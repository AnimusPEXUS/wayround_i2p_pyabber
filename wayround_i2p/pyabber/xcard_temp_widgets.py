...
for i in wayround_i2p.xmpp.xcard_temp.VCARD_ELEMENTS:
    ...
    if i[1] == wayround_i2p.xmpp.xcard_temp.PCData:
...
class {name}:
    ...
    def gen_data(self):
        ...
        if error:
            ...
            ret = wayround_i2p.xmpp.xcard_temp.PCData(
...
class {name}:
    ...
    def __init__(self, controller, data, editable):
        ...
        _init_fields(
            self,
            wayround_i2p.xmpp.xcard_temp.{name}_ELEMENTS,
            data,
            editable,
            {single_box}
            )
    ...
    def gen_data(self):
        ...
        if error:
            ...
            data = wayround_i2p.xmpp.xcard_temp.{prot_class_name}.new_empty()
...
def _init_fields(inst, elements_struct, data, editable, single_box):
    ...
    for i in elements_struct:
        ...
        if i[1] == wayround_i2p.xmpp.xcard_temp.Empty:
        ...
        elif i[1] == wayround_i2p.xmpp.xcard_temp.PCData:

            class_name = 'ValuePCDataWidget'

            if (i[0]
                == wayround_i2p.xmpp.xcard_temp.LXML_NAMESPACE + 'BINVAL'):
        ...
        if i[1] == wayround_i2p.xmpp.xcard_temp.Empty:
            ...
            if value:
                getattr(data, 'set_{}'.format(i[2]))(
                    wayround_i2p.xmpp.xcard_temp.Empty(
            ...
            if i[3] in ['', '?']:
                ...
                if not field.get_deleted():
                    attr(
                        wayround_i2p.xmpp.xcard_temp.PCData(
                            lxml.etree.QName(i[0]).localname,
                            field.get_value()
                            )
                        )
