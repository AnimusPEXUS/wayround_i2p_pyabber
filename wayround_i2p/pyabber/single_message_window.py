...
import wayround_i2p.pyabber.captcha
...
import wayround_i2p.xmpp.captcha
...
class SingleMessageWindow:
    ...
    def __init__(self, controller):
        ...
        self._thread_widget = wayround_i2p.pyabber.thread_widget.ThreadWidget(
            controller,
            contact_bare_jid=self._controller.jid.bare(),
            contact_resource=self._controller.jid.resource,
            operation_mode='normal'
        ...
        msg_edit_widget = \
            wayround_i2p.pyabber.message_edit_widget.MessageEdit(
                self._controller
                )
        ...
        if e != None:
            ...
            for i in e:
                if i.tag == '{urn:xmpp:captcha}captcha':
                    ...
                    captcha_data = \
                        wayround_i2p.xmpp.captcha.Captcha.new_from_element(i)

                    captcha_widget = \
                        wayround_i2p.pyabber.captcha.CAPTCHAWidget(
                            self._controller,
                ...
                if i.tag == '{jabber:x:data}x':
                    ...
                    xdata = wayround_i2p.xmpp.xdata.XData.new_from_element(i)

                    xform = wayround_i2p.pyabber.xdata.XDataFormWidget(
                        self._controller,
                        x_data=xdata,
                        origin_stanza=self._original_stanza,
                        editable=mode == 'new'
    ...
    def _on_send_button_clicked(self, button):
        ...
        if self._thread_frame_cb.get_active() == True:
            ...
            if t != None:
                thread = wayround_i2p.xmpp.core.MessageThread(
                    t
                    )
        ...
        for i in objects:
            if type(i) == wayround_i2p.xmpp.xdata.XData:
                i.set_typ('submit')
