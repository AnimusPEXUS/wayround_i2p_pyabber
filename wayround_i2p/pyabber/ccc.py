...
import lxml.etree
import wayround_i2p.pyabber.adhoc
import wayround_i2p.pyabber.bob
import wayround_i2p.pyabber.chat_window
import wayround_i2p.pyabber.contact_editor
import wayround_i2p.pyabber.disco
import wayround_i2p.pyabber.main
import wayround_i2p.pyabber.message_edit_widget
import wayround_i2p.pyabber.message_relay
import wayround_i2p.pyabber.muc
import wayround_i2p.pyabber.presence_control_window
import wayround_i2p.pyabber.registration
import wayround_i2p.pyabber.roster_storage
import wayround_i2p.pyabber.roster_window
import wayround_i2p.pyabber.single_message_window
import wayround_i2p.pyabber.subject_widget
import wayround_i2p.pyabber.thread_widget
import wayround_i2p.pyabber.xcard
import wayround_i2p.utils.gtk
import wayround_i2p.utils.threading
import wayround_i2p.xmpp.client
import wayround_i2p.xmpp.core
import wayround_i2p.xmpp.disco
import wayround_i2p.xmpp.muc
import wayround_i2p.xmpp.privacy
...
class ConnectionStatusMenu:
    ...
    def __init__(self, client_connetion_controller):
        ...
        self._on_roster_mi_activated_idle = \
            wayround_i2p.utils.gtk.to_idle(self._on_roster_mi_activated)

        self._on_messages_mi_activated_idle = \
            wayround_i2p.utils.gtk.to_idle(self._on_messages_mi_activated)
        ...
        self._on_reconnect_mi_activated_idle = \
            wayround_i2p.utils.gtk.to_idle(self._on_reconnect_mi_activated)
        self._on_disconnect_mi_activated_idle = \
            wayround_i2p.utils.gtk.to_idle(self._on_disconnect_mi_activated)
        ...
        self._on_destroy_mi_activated_idle = \
            wayround_i2p.utils.gtk.to_idle(self._on_destroy_mi_activated)
...
class ClientConnectionController:
    ...
    def __init__(self, main, profile, preset_name):
        """
        :param wayround_i2p.pyabber.main.ProfileSession profile:
        """

        if not isinstance(main, wayround_i2p.pyabber.main.Main):
            raise ValueError(
                "`main' must be wayround_i2p.pyabber.main.Main"
                )

        if not isinstance(profile, wayround_i2p.pyabber.main.ProfileSession):
            raise ValueError(
                "`profile' must be wayround_i2p.pyabber.main.ProfileSession"
                )
        ...
        self._rel_win_ctl = wayround_i2p.utils.gtk.RelatedWindowCollector()
...
""".format(i=i[0], s=i[1]))
        ...
        self.self_disco_info = wayround_i2p.xmpp.disco.IQDisco(mode='info')

        self.self_disco_info.set_identity(
            [
                wayround_i2p.xmpp.disco.IQDiscoIdentity(
                    'client', 'pc', 'pyabber'
                )
                ]
            )
...
#            )
    ...
    def _roster_window_constructor(self):
        return wayround_i2p.pyabber.roster_window.RosterWindow(self)

    def _disco_window_constructor(self):
        return wayround_i2p.pyabber.disco.Disco(self)

    def _add_mode_language_window_constructor(self):
        return wayround_i2p.pyabber.message_edit_widget.AddModeLang()

    def _adhoc_window_constructor(self):
        return wayround_i2p.pyabber.adhoc.AD_HOC_Window(self)

    def _adhoc_response_window_constructor(self):
        return wayround_i2p.pyabber.adhoc.AD_HOC_Response_Window(self)

    def _contact_editor_window_constructor(self):
        return wayround_i2p.pyabber.contact_editor.ContactEditor(self)

    def _presence_control_window_constructor(self):
        return \
            wayround_i2p.pyabber.presence_control_window.PresenceControlWindow(
                self
                )

    def _muc_jid_entry_dialog_constructor(self):
        return wayround_i2p.pyabber.muc.MUCJIDEntryDialog()

    def _muc_config_window_constructor(self):
        return wayround_i2p.pyabber.muc.MUCConfigWindow(self)

    def _muc_destruction_dialog_constructor(self):
        return wayround_i2p.pyabber.muc.MUCDestructionDialog(self)

    def _muc_identity_editor_window_constructor(self):
        return wayround_i2p.pyabber.muc.MUCIdentityEditorWindow(self)

    def _muc_join_dialog_constructor(self):
        return wayround_i2p.pyabber.muc.MUCJoinDialog(self)

    def _chat_window_constructor(self):
        return wayround_i2p.pyabber.chat_window.ChatWindow(self)

    def _single_message_window_constructor(self):
        return wayround_i2p.pyabber.single_message_window.SingleMessageWindow(
            self
            )

    def _registration_window_constructor(self):
        return wayround_i2p.pyabber.registration.RegistrationWindow(self)

    def _subject_edit_window_constructor(self):
        return wayround_i2p.pyabber.subject_widget.SubjectEditor(self)

    def _thread_edit_window_constructor(self):
        return wayround_i2p.pyabber.thread_widget.ThreadEditor(self)

    def _muc_voice_request_window_constructor(self):
        return wayround_i2p.pyabber.muc.MUCVoiceRequestWindow(self)

    def _muc_mini_identity_editor_window_constructor(self):
        return wayround_i2p.pyabber.muc.MUCMiniIdentityEditorWindow(self)

    def _xcard_window_constructor(self):
        return wayround_i2p.pyabber.xcard.XCardWindow(self)
...
""".format(i=i[0]))
    ...
    def connect(self):
        ...
        self.jid = wayround_i2p.xmpp.core.JID(
        ...
        self.message_relay = wayround_i2p.pyabber.message_relay.MessageRelay(
            self
            )

        self.client = wayround_i2p.xmpp.client.XMPPC2SClient(
            self.sock
            )

        self.roster_client = wayround_i2p.xmpp.client.Roster(
            self.client,
            self.jid
            )

        self.presence_client = wayround_i2p.xmpp.client.Presence(
            self.client,
            self.jid
            )

        self.message_client = wayround_i2p.xmpp.client.Message(
            self.client,
            self.jid
            )

        self.privacy_client = wayround_i2p.xmpp.privacy.PrivacyClient(
            self.client,
            self.jid
            )

        self.bob_mgr = wayround_i2p.pyabber.bob.BOBMgr(self)

#        self.muc_pool = wayround_i2p.pyabber.muc.MUCControllerPool(self)

        self._on_connection_event_idle = \
            wayround_i2p.utils.gtk.to_idle(self._on_connection_event)
        ...
        self._on_stream_io_event_idle = \
            wayround_i2p.utils.gtk.to_idle(self._on_stream_io_event)
        ...
        features_waiter = wayround_i2p.utils.threading.SignalWaiter(
            self.client.signal,
            'features'
            )
        ...
        if auto:
            ...
            if (not self._disconnection_flag.is_set()
                ...
                res = wayround_i2p.xmpp.client.drive_starttls(
                    self.client,
                    last_features,
                    self.jid.bare(),
                    self.connection_info.host,
                ...
                if not wayround_i2p.xmpp.core.is_features_element(res):
            ...
            if (not self._disconnection_flag.is_set()
                ...
                res = wayround_i2p.xmpp.client.drive_sasl(
                    self.client,
                    last_features,
                    self.jid.bare(),
                    self.connection_info.host,
            ...
            if (not self._disconnection_flag.is_set()
                ...
                res = wayround_i2p.xmpp.client.bind(
                    self.client,
                    self.jid.resource
                    )
                if not isinstance(res, str):
                    ...
                    self.jid.update(
                        wayround_i2p.xmpp.core.JID.new_from_str(res)
                        )
                    logging.debug(
            ...
            if (not self._disconnection_flag.is_set()
                ...
                res = wayround_i2p.xmpp.client.session(
                    self.client,
                    self.jid.domain
                    )

                if (not isinstance(res, wayround_i2p.xmpp.core.Stanza)
                        or res.is_error()):
            ...
            if (not self._disconnection_flag.is_set()
                ...
                self.roster_storage = \
                    wayround_i2p.pyabber.roster_storage.RosterStorage(
                        self.jid,
                        self.roster_client,
                ...
                self._on_presence_idle = \
                    wayround_i2p.utils.gtk.to_idle(self._on_presence)
                ...
                self.disco_service = wayround_i2p.xmpp.disco.DiscoService(
                ...
                self._message_relay_listener_idle = \
                    wayround_i2p.utils.gtk.to_idle(
            ...
            if event == 'new_message':
                if type_ in [
                    ...
                    if type_ == 'message_chat':
                        ...
                        for i in cp.pages:
                            if (isinstance(i, wayround_i2p.pyabber.chat_pager.GroupChat)
    ...
    def _on_presence(self, event, presence_obj, from_jid, to_jid, stanza):
        ...
        with self._incomming_message_lock:
            ...
            if event == 'presence':
                if wayround_i2p.xmpp.muc.has_muc_elements(
                        stanza.get_element()
                        ):
                    message_relay_listener_call_queue = \
                        self.message_relay.signal.gen_call_queue(
                    ...
                    self.show_chat_window()
                    w = self.get_chat_window()
                    jid = wayround_i2p.xmpp.core.JID.new_from_str(
                        stanza.get_from_jid()
                        )
