...
import wayround_i2p.pyabber.misc
import wayround_i2p.pyabber.muc_roster_storage
import wayround_i2p.pyabber.xdata
...
for i in wayround_i2p.xmpp.muc.ROLES:
    ROLE_LISTSTORE.append([i])

for i in wayround_i2p.xmpp.muc.AFFILIATIONS:
...
class MUCMiniIdentityEditorWindow:
    ...
    def __init__(self, controller):
        ...
        self._iterated_loop = wayround_i2p.utils.gtk.GtkIteratedLoop()
    ...
    def _on_request_clicked(self, button):
        ...
        if self._actor_cb.get_active():
            ...
            if actor != None:
                actor = wayround_i2p.xmpp.muc.Actor(actor)
        ...
        query = wayround_i2p.xmpp.muc.Query(
            xmlns='#admin',
            item=items
            )

        stanza = wayround_i2p.xmpp.core.Stanza('iq')
...
class MUCVoiceRequestWindow:
    ...
    def _on_request_clicked(self, button):
        ...
        fields = [
            wayround_i2p.xmpp.xdata.XDataField(
                var='FORM_TYPE',
                values=[
                    wayround_i2p.xmpp.xdata.XDataValue(
        ...
        xdata = wayround_i2p.xmpp.xdata.XData(
            typ='submit',
            fields=fields
            )

        stanza = wayround_i2p.xmpp.core.Stanza(tag='message')
        stanza.set_from_jid(str(self._controller.jid))
...
class MUCJoinDialog:
    ...
    def _on_join_clicked(self, button):
        ...
        o = wayround_i2p.xmpp.core.JID.new_from_str(self._entry.get_text())
...
class MUCJIDEntryDialog:
    ...
    def __init__(self):
        ...
        self._iteration_loop = wayround_i2p.utils.gtk.GtkIteratedLoop()
...
class MUCConfigWindow:
    ...
    def run(self, stanza):
        ...
        if not isinstance(stanza, wayround_i2p.xmpp.core.Stanza):
            raise ValueError(
                "`xdata' must be wayround_i2p.xmpp.core.Stanza inst"
                )
        ...
        if q == None:
            ...
            query = wayround_i2p.xmpp.muc.Query.new_from_element(q)
            x = query.get_xdata()
        ...
        if error_result:
            ...
            xdata_widget_ctl = (
                wayround_i2p.pyabber.xdata.XDataFormWidget(
                    self._controller,
                    xdata,
                    stanza
                    )
    ...
    def _on_submit_pressed(self, button):
        ...
        if not self._form_controller:
            ...
            if x_data == None:
                ...
                wayround_i2p.xmpp.muc.submit_room_configuration(
...
class MUCDestructionDialog:
    ...
    def _on_submit_button_clicked(self, button):
        ...
        res = wayround_i2p.xmpp.muc.destroy_room(
...
class MUCPopupMenu:
    ...
    def set(self, muc_jid):
        ...
        jid = wayround_i2p.xmpp.core.JID.new_from_string(muc_jid)
    ...
    def _on_new_muc_inst_mi_activated(self, menuitem):
        ...
        if jid != None:
            ...
            res = wayround_i2p.xmpp.muc.create_room_instantly(
                room_bare_jid=jid,
                from_full_jid=self._own_jid.full(),
                stanza_processor=self._client.stanza_processor
                )
    ...
    def _on_new_muc_conf_mi_activated(self, menuitem):
        ...
        if jid != None:
            ...
            res = wayround_i2p.xmpp.muc.request_room_configuration(
                room_bare_jid=jid,
                from_full_jid=self._own_jid.full(),
                stanza_processor=self._client.stanza_processor
                )
    ...
    def _on_discover_nick_mi_activated(self, menuitem):
        ...
        if jid != None:
            ...
            res = wayround_i2p.xmpp.muc.discover_room_nickname(
                room_bare_jid=jid,
                from_full_jid=self._own_jid.full(),
                stanza_processor=self._stanza_processor
                )
            ...
            if isinstance(res, wayround_i2p.xmpp.core.Stanza):
                if res.is_error():
                    ...
                    if q != None:
                        q = wayround_i2p.xmpp.disco.IQDisco.new_from_element(q)
                ...
                if not error:
                    ...
                    if isinstance(res, wayround_i2p.xmpp.core.Stanza):
                        ...
                        if muc_q is None:
                            ...
                            q = wayround_i2p.xmpp.muc.Query.new_from_element(
                                muc_q
                                )
                            data = []
                            for i in q.get_item():
