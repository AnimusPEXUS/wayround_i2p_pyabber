...
import wayround_i2p.pyabber.chat_log_widget
import wayround_i2p.pyabber.jid_widget
import wayround_i2p.pyabber.message_edit_widget
import wayround_i2p.pyabber.muc_roster_widget
...
class Chat:
    ...
    def __init__(
        ...
        self._log = wayround_i2p.pyabber.chat_log_widget.ChatLogWidget(
            self._controller,
            self,
            mode,
        ...
        self._editor = wayround_i2p.pyabber.message_edit_widget.MessageEdit(
            self._controller
            )
        ...
        if mode == 'chat':
            jid_widget = wayround_i2p.pyabber.jid_widget.JIDWidget(
                controller,
                controller.roster_storage,
                contact_bare_jid
                )
        else:
            jid_widget = wayround_i2p.pyabber.jid_widget.MUCRosterJIDWidget(
                contact_bare_jid,
        ...
        self._subject_widget = \
            wayround_i2p.pyabber.subject_widget.SubjectWidget(
        ...
        self._queues.append(_t)
        self._thread_widget = \
            wayround_i2p.pyabber.thread_widget.ThreadWidget(
                controller, contact_bare_jid, contact_resource, mode,
                message_relay_listener_call_queue=_t
                )
        ...
        self._message_relay_listener_idle = \
            wayround_i2p.utils.gtk.to_idle(self._message_relay_listener)
    ...
    def make_available(self):
        j = wayround_i2p.xmpp.core.JID.new_from_str(self.contact_bare_jid)
    ...
    def send_message(self):
        ...
        if self._mode != 'groupchat':
            ...
            jid_obj = wayround_i2p.xmpp.core.JID.new_from_str(
...
class ChatPager:
    ...
    def add_private(self, full_jid):
        ret = None
        j = wayround_i2p.xmpp.core.JID.new_from_string(full_jid)
        j2 = j.copy()
        j2.resource = None
    ...
    def search_page(self, jid_obj, thread_id=None, type_=None):
        ...
        if not isinstance(jid_obj, wayround_i2p.xmpp.core.JID):
            raise ValueError("`jid_obj' must be wayround_i2p.xmpp.core.JID")
...
class GroupChat:
    ...
    def __init__(
        ...
        with self._lock:
            ...
            self._room_bare_jid_obj = wayround_i2p.xmpp.core.JID.new_from_str(
                room_bare_jid
                )
            ...
            self._roster_widget = \
                wayround_i2p.pyabber.muc_roster_widget.MUCRosterWidget(
            ...
            self._tab_widget = \
                wayround_i2p.pyabber.jid_widget.GroupChatTabWidget(
                    room_bare_jid,
                    own_resource,
                    self._controller,
                    self._storage,
