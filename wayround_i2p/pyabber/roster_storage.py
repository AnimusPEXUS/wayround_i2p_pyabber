...
class RosterStorage:
    ...
    def __init__(self, own_jid_obj, roster_client, presence_client):
        ...
        if not isinstance(roster_client, wayround_i2p.xmpp.client.Roster):
            raise TypeError(
                "`roster_client' must be wayround_i2p.xmpp.client.Roster"
                )
        ...
        if not isinstance(own_jid_obj, wayround_i2p.xmpp.core.JID):
            raise TypeError(
                "`own_jid_obj' must be wayround_i2p.xmpp.core.JID"
                )
    ...
    def load_from_server(self):
        ...
        if res is None:
            ...
            elif (isinstance(res, wayround_i2p.xmpp.core.Stanza)
    ...
    def _on_presence(self, event, presence_obj, from_jid, to_jid, stanza):
        ...
        if event == 'presence':
            ...
            if not wayround_i2p.xmpp.muc.has_muc_elements(
                    stanza.get_element()
                    ):
                ...
                if not stanza.get_typ() in [
                    ...
                    if from_jid:
                        f_jid = wayround_i2p.xmpp.core.JID.new_from_str(
                            from_jid
                            )
                    else:
        ...
        with self._muc_storage_creation_lock:
            ...
            if not f_jid_bare in self._muc_rosters:
                self._muc_rosters[f_jid_bare] = \
                    wayround_i2p.pyabber.muc_roster_storage.Storage(
                        f_jid.bare_obj(),
