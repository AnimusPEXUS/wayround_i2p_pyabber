...
class PrivacyEditor:
    ...
    def __init__(self, to_jid, from_jid, stanza_processor):
        ...
        if not isinstance(from_jid, wayround_i2p.xmpp.core.JID):
            raise ValueError("`from_jid' must be wayround_i2p.xmpp.core.JID")

        if not isinstance(
            stanza_processor,
            wayround_i2p.xmpp.core.StanzaProcessor
            ):
            raise ValueError(
        "`stanza_processor' must be wayround_i2p.xmpp.core.StanzaProcessor"
                )
    ...
    def set_jid(self, jid):
        ...
        try:
            jid = wayround_i2p.xmpp.core.JID.new_from_string(jid)
        except:
            ...
            res = wayround_i2p.xmpp.privacy.get_privacy_lists(
                to_jid=jid,
                from_jid=str(self._from_jid),
                stanza_processor=self._stanza_processor
                )

            if isinstance(res, wayround_i2p.xmpp.core.StanzaError):
                wayround_i2p.pyabber.misc.stanza_error_error_message(
                    self._window,
                    res,
    ...
    def _on_active_save_button_clicked(self, button):
        ...
        res = wayround_i2p.xmpp.privacy.set_active_list(
    ...
    def _on_default_save_button_clicked(self, button):
        ...
        res = wayround_i2p.xmpp.privacy.set_default_list(
            value, self._to_jid, str(self._from_jid), self._stanza_processor
            )
            ...
            if not wayround_i2p.utils.types.struct_check(
                ...
                if numbers_err:
                    ...
                    for i in data:
                        ...
                        item = wayround_i2p.xmpp.privacy.Item(
                            i['action'], i['order']
                            )
                    ...
                    res = wayround_i2p.xmpp.privacy.set_list(
                        name,
    ...
    def _open_edit_list(self, name):
        ...
        res = wayround_i2p.xmpp.privacy.get_list(
    ...
    def _on_delete_list_button_clicked(self, button):
        ...
        if iter is None:
            ...
            res = wayround_i2p.xmpp.privacy.delete_list(
                name, self._to_jid, str(self._from_jid), self._stanza_processor
                )
