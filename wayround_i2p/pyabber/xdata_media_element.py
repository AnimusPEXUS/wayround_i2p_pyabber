...
import wayround_i2p.xmpp.bob
import wayround_i2p.xmpp.xdata_media_element
...
class MediaElementWidget:
    ...
    "`controller' must be wayround_i2p.pyabber.ccc.ClientConnectionController"
        ...
        if not isinstance(
            media_data,
            wayround_i2p.xmpp.xdata_media_element.Media
            ):
            raise TypeError(
    "`media_data' must be wayround_i2p.xmpp.xdata_media_element.Media"
                )
...
class URIWidget:
    ...
    "`controller' must be wayround_i2p.pyabber.ccc.ClientConnectionController"
        ...
        if not isinstance(
            uri_data,
            wayround_i2p.xmpp.xdata_media_element.URI
            ):
            raise TypeError(
    "`media_data' must be wayround_i2p.xmpp.xdata_media_element.URI"
                )
        ...
        cid_parsed = wayround_i2p.xmpp.bob.parse_cid(cid)
        ...
        if is_cid_picture:
            b.set_orientation(Gtk.Orientation.VERTICAL)
            self._bob_widget = wayround_i2p.pyabber.bob.BOBWidget(controller)
            self._bob_widget.set_data(
                cid,
                wayround_i2p.xmpp.core.JID.new_from_str(
