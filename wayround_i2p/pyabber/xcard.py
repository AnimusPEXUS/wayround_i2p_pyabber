...
import wayround_i2p.pyabber.xcard_temp_widgets
import wayround_i2p.utils.gtk
import wayround_i2p.xmpp.core
import wayround_i2p.xmpp.xcard_4
import wayround_i2p.xmpp.xcard_temp
...
class XCardWidget:
    ...
    def __init__(self, controller):
        ...
        for i in wayround_i2p.xmpp.xcard_temp.VCARD_ELEMENTS:
...
#        for i in wayround_i2p.xmpp.xcard_4.VCARD_ELEMENTS:
#            mi = Gtk.MenuItem(i[0].split('}')[1])
#            mi.set_no_show_all(True)
#            mi.connect('activate', self._on_add_mi_activate, i[0])
#            self._vcard_temp_menu_items.append(mi)
#            add_menu.append(mi)
    ...
    def set_xcard(self, obj):
        ...
        if isinstance(obj, wayround_i2p.xmpp.xcard_temp.XCardTemp):
            self._mode_label.set_text("xcard-temp mode")
        elif isinstance(obj, wayround_i2p.xmpp.xcard_4.XCard):
        ...
        for i in order:
            ...
            if elem_obj_typ.__module__ == 'wayround_i2p.xmpp.xcard_temp':
                try:
                    cls = getattr(
                        wayround_i2p.pyabber.xcard_temp_widgets,
            ...
            elif elem_obj_typ.__module__ == 'wayround_i2p.xmpp.xcard_4':
                try:
                    cls = getattr(
                        wayround_i2p.pyabber.xcard_4_widgets,
                        elem_obj.corresponding_tag()
                        )
                    cw = cls(
                        self._controller,
            ...
            if error:
                d = wayround_i2p.utils.gtk.MessageDialog(
                    wayround_i2p.utils.gtk.get_root_gtk_window(
                        self.get_widget()
                        ),
                    0,
    ...
    def gen_xcard(self):
        ...
        elems = []
        if isinstance(self._data, wayround_i2p.xmpp.xcard_temp.XCardTemp):
            elems = wayround_i2p.xmpp.xcard_temp.VCARD_ELEMENTS
        elif isinstance(self._data, wayround_i2p.xmpp.xcard_4.XCard):
            elems = wayround_i2p.xmpp.xcard_4.VCARD_ELEMENTS
        else:
    ...
    def _gen_xcard_part_two(self, data):
            ...
            if obj == None:
                ...
                if obj_data == None:
                    ...
                    if (obj_data_type.__module__
                        == 'wayround_i2p.xmpp.xcard_temp'):

                        for j in  wayround_i2p.xmpp.xcard_temp.VCARD_ELEMENTS:
                        ...
                        == 'wayround_i2p.xmpp.xcard_4'):

                        for j in  wayround_i2p.xmpp.xcard_4.VCARD_ELEMENTS:
                            if (lxml.etree.QName(j[0]).localname
                                == obj_data.corresponding_tag()):
        ...
        if not cancel:
            xcard = wayround_i2p.xmpp.xcard_temp.XCardTemp()
    ...
    def _on_add_mi_activate(self, mi, name):
        tag, ns = wayround_i2p.utils.lxml.parse_tag(name, None, None)

        error = False
        cw = None

        if ns == wayround_i2p.xmpp.xcard_temp.NAMESPACE:
            try:
                cls = getattr(
                    wayround_i2p.pyabber.xcard_temp_widgets,
                    tag.replace('-', '')
                ...
                if cls2 == wayround_i2p.xmpp.xcard_temp.PCData:
                    cw = cls(
                        self._controller,
                        cls2(tag, ''),
                        self.get_editable()
        ...
        elif ns == wayround_i2p.xmpp.xcard_4.NAMESPACE:
            try:
                cls = getattr(
                    wayround_i2p.pyabber.xcard_4_widgets,
                    tag.replace('-', '')
...
class XCardWindow:
    ...
    def __init__(self, controller):
        ...
        if not isinstance(
            controller,
            wayround_i2p.pyabber.ccc.ClientConnectionController
            ):
            raise ValueError(
                "`controller' must be wayround_i2p.xmpp.client.XMPPC2SClient"
                )
    ...
    def _on_reload_button_clicked(self, button):
        ...
        if mode == 'vcard-temp':
            objects.append(wayround_i2p.xmpp.xcard_temp.XCardTemp())
        elif mode == 'vcard-4.0':
            objects.append(wayround_i2p.xmpp.xcard_4.XCard())
        ...
        wayround_i2p.utils.gtk.Waiter.wait_thread(t)
        ...
        if res == None:
            ...
            if res.is_error():
                wayround_i2p.pyabber.misc.stanza_error_message(
                    self._window,
                    res,
                ...
                if mode == 'vcard-temp':
                    ...
                    for i in res.get_element():
                        if wayround_i2p.utils.lxml.is_lxml_tag_element(i):
                            if wayround_i2p.xmpp.xcard_temp.is_xcard(i):
                                try:
                                    obj = \
                                        wayround_i2p.xmpp.xcard_temp.\
                                            XCardTemp.\
    ...
    def _on_new_button_clicked(self, button):
        ...
        if mode == 'vcard-temp':
            self._xcard_widget.set_xcard(
                wayround_i2p.xmpp.xcard_temp.XCardTemp()
                )
        elif mode == 'vcard-4.0':
            self._xcard_widget.set_xcard(wayround_i2p.xmpp.xcard_4.XCard())
