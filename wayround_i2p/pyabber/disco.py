...
import wayround_i2p.pyabber.privacy
...
class DiscoMenu:
    ...
    def __init__(self, controller):
        ...
        mucmenu = wayround_i2p.pyabber.muc.MUCPopupMenu(
    ...
    def set(self, target_jid_str, node=None):
        ...
        q, stanza = wayround_i2p.xmpp.disco.get_info(
...
#        privacy_mi.set_sensitive(False)
        ...
        if isinstance(stanza, wayround_i2p.xmpp.core.Stanza):
            if stanza.is_error():
    ...
    def _on_commands_mi_activated(self, menuitem):
        ...
        wayround_i2p.pyabber.adhoc.adhoc_window_for_jid_and_node(
    ...
    def _on_privacy_mi_activated(self, menuitem):
        ...
        w = wayround_i2p.pyabber.privacy.PrivacyEditor(
            to_jid=self._target_jid_str,
    ...
    def _on_registration_mi_activated(self, menuitem):
        ...
        self._controller.show_registration_window(
            target_jid_obj=wayround_i2p.xmpp.core.JID.new_from_str(
                self._target_jid_str
                ),
...
class Disco:
    ...
    def __init__(self, controller):
        ...
        self._on_destroy_idle = \
            wayround_i2p.utils.gtk.to_idle(self._on_destroy)
        ...
        self._on_go_button_pressed_idle = \
            wayround_i2p.utils.gtk.to_idle(self._on_go_button_pressed)
        ...
        self._on_jid_or_node_entry_activate_idle = \
            wayround_i2p.utils.gtk.to_idle(self._on_jid_or_node_entry_activate)
...
#            wayround_i2p.utils.gtk.to_idle(self._on_row_activated)
        view_tw.connect('row-activated', self._on_row_activated)

#        self._on_treeview_buttonpress_idle = \
#            wayround_i2p.utils.gtk.to_idle(self._on_treeview_buttonpress)
        view_tw.connect(
            'button-release-event',
            self._on_treeview_buttonpress
            )

#        self._fill2_idle = wayround_i2p.utils.gtk.to_idle(self._fill2)
...
#        t.start()
#
#        w = wayround_i2p.utils.gtk.Waiter(t.join, None, t.is_alive)
#        w.wait()
...
def _t_proc(x, y):
    x['data'] = wayround_i2p.xmpp.disco.get(**y)
